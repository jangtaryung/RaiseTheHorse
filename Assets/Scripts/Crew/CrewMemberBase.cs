...
public abstract class CrewMemberBase
{
    ...
    /// <summary>레벨업이 발생할 때마다 발행됩니다. 외부 시스템(업그레이드 UI, 이펙트 등)에서 구독 가능.</summary>
    ...
    /// <summary>BaseAttack에 delta를 더합니다. 음수도 허용(최솟값 0).</summary>
    public void ApplyAttackBonus(float delta)
    {
        BaseAttack = Mathf.Max(0f, BaseAttack + delta);
    }

    /// <summary>BaseDefense에 delta를 더합니다.</summary>
    public void ApplyDefenseBonus(float delta)
    {
        BaseDefense = Mathf.Max(0f, BaseDefense + delta);
    }

    /// <summary>BaseMaxHP에 delta를 더합니다.</summary>
    public void ApplyMaxHPBonus(float delta)
    {
        BaseMaxHP = Mathf.Max(1f, BaseMaxHP + delta);
    }
}
